  ...
  public:
    ...
    class CompatiblePlacementTable
    {
      public:
        ...
        {
            if (sharedCellBELTypeName2ID.find(tmpStr) == sharedCellBELTypeName2ID.end()) {
                std::cout << tmpStr << " " << sharedCellBELTypeName2ID.size() << std::endl;
                for (auto onePair : sharedCellBELTypeName2ID) {
                    std::cout << onePair.first << " " << onePair.second << std::endl;
                }
            }
    ...
    class PlacementUnit
    {
      public:
        ...
        // Compared with v1, the v2 calculates the average between the location of WL minimization and spreading
        inline void setSpreadLocationv2(float x, float y, float forgetRatio)
        {
            assert(!locked);

            if(lastWLminX > -100 && lastWLminY > -100)
            {
                assert(forgetRatio <= 1);
                if(lastWLminX > -100 && lastWLminY > -100)
                {
                    assert(forgetRatio <= 1);
                    x = x * forgetRatio + lastWLminX*(1-forgetRatio);
                    y = y * forgetRatio + lastWLminY*(1-forgetRatio);
                    lastSpreadX = x;
                    lastSpreadY = y;
                }
                lastAnchorX = x;
        ...
        inline void recordWLminLocation()
        {
            lastWLminX = anchorX;
            lastWLminY = anchorY;
        }
      ...
      private:
        ...
        // record the (X,Y) after the wirelength minimization
        float lastWLminX = -2000, lastWLminY = -2000;
    ...
    class PlacementMacro : public PlacementUnit
    {
      public:
        ...
        inline void addCell(DesignInfo::DesignCell *curCell, DesignInfo::DesignCellType cellType, float x, float y)
        {
            ...
            auto CellinMacroCmp = [](std::pair<float, int> cell1, std::pair<float, int> cell2)
            {
                return cell1.first < cell2.first;
            };

            // Order the cellsInMacro
            std::vector<std::pair<float, int>> ordered_cellYList;
            for(int i=0; i<offsetY.size(); i++)
            {
                ordered_cellYList.push_back(std::make_pair(offsetY[i], i));
            }
            std::sort(ordered_cellYList.begin(), ordered_cellYList.end(), CellinMacroCmp);
            
            std::vector<DesignInfo::DesignCell *> cellsInMacrotmp;
            std::vector<DesignInfo::DesignCellType> cells_Typetmp;
            std::vector<float> offsetXtmp;
            std::vector<float> offsetYtmp;

            for(int i=0; i < ordered_cellYList.size(); i++)
            {
                int ordered_id = ordered_cellYList[i].second;
                cellsInMacrotmp.push_back(cellsInMacro[ordered_id]);
                cells_Typetmp.push_back(cells_Type[ordered_id]);
                offsetXtmp.push_back(offsetX[ordered_id]);
                offsetYtmp.push_back(offsetY[ordered_id]);                
            }

            cellsInMacro = cellsInMacrotmp;
            cells_Type = cells_Typetmp;
            offsetX = offsetXtmp;
            offsetY = offsetYtmp;
		...
		/**
		 * @brief set the reference cell's name
		 * 
		 * @param cellname 
		 */
		inline void SetreferenceCell(std::string cellname)
        {
            refCellName = cellname;
        }
		/**
		 * @brief Get the reference cell's name
		 * 
		 * @return std::string 
		 */
        inline std::string GetreferenceCell()
        {
            return refCellName;
        }
        /**
    ...
    void printStat(bool verbose = true);
	...
	const void outputCongestionMap(int iter = -1);
                                                                            ...
                                                                            int siteNumThreshold)
    {
        ...
        auto celltmp = getDesignInfo()->getCell(celltmpname);
        std::cout<<celltmpname<<": "<<celltmp->hasRegionConstraint()<<std::endl;
        std::cout<<celltmpname<<": ";
        for(auto regionconstr : getDesignInfo()->GetRegionConstraintAll())
        {
        ...
        */
                ...
                while (binXYqueue.size() > 0)
                {
                    ...
                    for (auto curSite : curBin->getCorrespondingSites())
                    {
                        if (!curSite->isOccupied() && !curSite->isMapped())
                        {
                            ...
                            {
                                // Consider the regional constraint
                                ...
                                if(curCell->hasRegionConstraint())
                                {
                                    if(curCell->IsinRegionConstraint(locX, locY))
                                    {
                                        bool flag_overflow = false;
                                        for(auto regionconstr : designInfo->GetRegionConstraintAll())
                                        {
                                            if((!regionconstr->hasRegionConstrainedCell(curCell)) && (regionconstr->IsinRegionConstraint(locX, locY, left_right_slack, up_down_slack)))
                                            {
        ...
        //std::cout<<curCell->getName()<<" "<<res->size()<<" "<<displacementThreshold<<std::endl;
...
#pragma omp parallel for
	...
	/**
	 * @brief generate output for mlcad
	 * 
	 * @param placefilename: pl file
	 * @param placefilename2: tcl file
	 */
    void PlacementLocationOutput(std::string plFileName, std::string tclFileName)
    {
        std::ofstream f_out, f_out_tcl;
        f_out.open(plFileName);
		// f_out_tcl.open(tclFileName);

        std::vector<int> dspXMap = {2, 20, 26, 30, 38, 44, 53, 63, 75, 98, 108, 114, 123, 133, 145, 168, 174, 186, 202};
        std::vector<int> bramXMap = {11, 17, 35, 60, 72, 101, 105, 130, 142, 171, 189, 195};
        std::vector<std::vector<std::vector<std::string>>> siteCells(206, 
            std::vector<std::vector<std::string>>(300, std::vector<std::string>()));
        int maxBRAMY = 1000000;
        int maxDSPY = 1000000;

		f_out_tcl << "place_cell { \\\n";
        for (auto &DSPBRAM_LegalSitePair : getPULegalSite())
        {
            std::string placementStr = "";
            std::string placementStrTcl = "";
            if (auto tmpMacro = dynamic_cast<PlacementInfo::PlacementMacro *>(DSPBRAM_LegalSitePair.first))
            {
                if (tmpMacro->getMacroType() == PlacementInfo::PlacementMacro::PlacementMacroType_BRAM)
                {
                    ...
                    //{
                    auto curCell = tmpMacro->getCells()[0];
                    auto targetSite = DSPBRAM_LegalSitePair.second[0];
                    if (!curCell->isVirtualCell())
                    {
                        if (curCell->getOriCellType() == DesignInfo::CellType_RAMB36E2)
                        {
                            // std::cout << "Pair: " << curCell->getName() << " " << targetSite->getName() << std::endl;
                            assert(targetSite->getSiteY() % 2 == 0);
                            int x = targetSite->getSiteX();
                            int y = targetSite->getSiteY() / 2;
							std::string realSiteName = "RAMB36_X" + std::to_string(x) + "Y" + std::to_string(y);
                            x = bramXMap[x]; // sitemap
                            y = y * 5; // sitemap
                            assert(x <= 205 && y <= 295);
                            siteCells[x][y].emplace_back(curCell->getName());
                            placementStr += curCell->getName() + " " + std::to_string(x) + " " + std::to_string(y) + " 0\n";
							placementStrTcl += "  " + curCell->getName() + " " + realSiteName + " \\\n";
                        }
                        else if (curCell->getOriCellType() == DesignInfo::CellType_RAMB18E2)
                        {
                            std::cout << "!!!!!!!! !!!!!!!" << " RAMB18E2 found!" << std::endl;
                            exit(1);
                        }
                        else
                        {
                ...
                else if (tmpMacro->getMacroType() == PlacementInfo::PlacementMacro::PlacementMacroType_DSP)
                {
                    ...
                    //{
                        ...
                        if (!curCell->isVirtualCell())
                        {
                            ...
                            siteCells[x][y].emplace_back(curCell->getName());
                            placementStr += curCell->getName() + " " +  std::to_string(x) + " " + std::to_string(y) + " 0\n";
							placementStrTcl += "  " + curCell->getName() + " " + targetSite->getName() + " \\\n";
                        }
                        else
                        {
            ...
            {
                f_out << placementStr;
				f_out_tcl << placementStrTcl;
            }
        }

		// ouput IO
		if (JSONCfg["writePort"] == "true") {
			auto fixedPU = getFixedPlacementUnits();
			std::cout << "Add port placement into tcl script. PU num: " << fixedPU.size() << " expect: " << getFixedPUNum() << std::endl;
			assert(fixedPU.size() == getFixedPUNum());

	        std::ofstream f_out, f_out_db;
			f_out_db.open(tclFileName + ".debug");
			for (auto pu : fixedPU) {
				PlacementUnpackedCell *curCell= dynamic_cast<PlacementUnpackedCell *>(pu);
				assert(curCell); // exist
				f_out_tcl << "  " << curCell->getCell()->getName() << " " << curCell->getFixedSiteName() << " \\\n";
				f_out_db << "  " << curCell->getCell()->getName() << " " << curCell->getFixedSiteName() << " \\\n";
			}
			f_out_db.close();
		}
		f_out_tcl << "}";
        // std::map<PlacementUnit *, float> &PUX = getPULegalXY().first;
        // std::map<PlacementUnit *, float> &PUY = getPULegalXY().second;
        // for(auto pairPUX : PUX)
        // {
        //     PlacementMacro* pairmacro = dynamic_cast<PlacementMacro*>(pairPUX.first);
        //     f_out<<pairmacro->GetreferenceCell()<<' '<<pairPUX.second<<' '<<PUY[pairPUX.first]<<" 0"<<std::endl;
        // }
        for (int i = 0; i < siteCells.size(); i ++) {
            for (int j = 0; j < siteCells[0].size(); j ++) {
                if (siteCells[i][j].size() > 1) {
                    std::cout << "(" << i << ", " << j << ") ";
                    for (auto s : siteCells[i][j])
                        std::cout << s << " ";
                    std::cout << std::endl;
                }
            }
    ...
    /**
     * @brief fixedPUNum: io num for mlcad
     *
     */
	void setFixedPUNum(int fixedPUNum_) 
	{
		fixedPUNum = fixedPUNum_;
	}
  ...
  private:
	...
	/**
	 * @brief The number of fixed cell (IO num for mlcad)
	 * 
	 */
	int fixedPUNum = 0;
    /**
