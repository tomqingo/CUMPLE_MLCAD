      ...
      binGrid(placementInfo->getBinGrid(placementInfo->getSharedBELTypeId(sharedCellType)))
{
    ...
    if (JSONCfg.find("SpreaderInnerIter") != JSONCfg.end())
    {
...
void GeneralSpreader::spreadPlacementUnits(float forgetRatio)
{
    ...
    while (true)
    {
            ...
            float improved = std::fabs((historyTotalCellNum[historyTotalCellNum.size() - 1] +
                                       ...
                                       (historyTotalCellNum[historyTotalCellNum.size() - 20] +
                                        ...
                                        historyTotalCellNum[historyTotalCellNum.size() - 18]) /
                                           3);
            if ((improved < 40 && historyTotalCellNum[historyTotalCellNum.size() - 1] < 1000 && loopCnt > 150) ||
                loopCnt > 250)
            {
                break;
            }
        ...
        //if (nJobs > 4)
        omp_set_num_threads(nJobs); // we don't need that high parallelism here since regionNum will be small in later iterations
...
void GeneralSpreader::SpreadRegion::SubBox::spreadCellsH(SubBox **boxA, SubBox **boxB)
{
    ...
    *boxB = new SubBox(this, topBinY, bottomBinY, boxBLeft, boxBRight, cutLineIdX + 1, cellIds.size() - 1);
    std::vector<PlacementInfo::Location> &cellLoc = placementInfo->getCellId2location();
    std::vector<DesignInfo::DesignCell*> &cellcol = placementInfo->getCells();
        ...
        for (int binX = boxARight; binX >= boxALeft; binX--)
        {
            ...
            if (num > 0)
            {
                ...
                if (fabs(rangeold) < 1e-5)
                {
                    ...
                    {
                        if(GP2Region && cellcol[boxACellIds[ci]]->hasRegionConstraint())
                        {
                            double dis_orgLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintX(cellLoc[boxACellIds[ci]].X);
                            double dis_newLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintX(newLoc);

                            if(dis_orgLoc>=dis_newLoc)
                ...
                else
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                        ...
                        {
                            double dis_orgLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintX(cellLoc[boxACellIds[ci]].X);
                            double dis_newLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintX(newLoX + (cellLoc[boxACellIds[ci]].X - oldLoX) * scale);
        ...
        for (int binX = boxBLeft; binX <= boxBRight; binX++)
        {
            ...
            if (num > 0)
            {
                ...
                if (fabs(rangeold) < 1e-5)
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                        if(GP2Region && cellcol[boxBCellIds[ci]]->hasRegionConstraint())
                        {
                            double dis_orgLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintX(cellLoc[boxBCellIds[ci]].X);
                            double dis_newLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintX(newLoc);
                ...
                else
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                        ...
                        {
                            double dis_orgLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintX(cellLoc[boxBCellIds[ci]].X);
                            double dis_newLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintX(newLoX + (cellLoc[boxBCellIds[ci]].X - oldLoX) * scale);
            ...
            if (num > 0)
            {
                ...
                if (fabs(rangeold) < 1e-5)
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                        if(GP2Region && cellcol[boxACellIds[ci]]->hasRegionConstraint())
                        {
                            double dis_orgLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintY(cellLoc[boxACellIds[ci]].Y);
                            double dis_newLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintY(newBottomc);
                ...
                else
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                        ...
                        {
                            double dis_orgLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintY(cellLoc[boxACellIds[ci]].Y);
                            double dis_newLoc = cellcol[boxACellIds[ci]]->calDisToRegionConstraintY(newBottomY + (cellLoc[boxACellIds[ci]].Y - oriBottomY) * scale);
            ...
            if (num > 0)
            {
                ...
                if (fabs(rangeold) < 1e-5)
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                            if(GP2Region && cellcol[boxBCellIds[ci]]->hasRegionConstraint())
                            {
                                double dis_orgLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintY(cellLoc[boxBCellIds[ci]].Y);
                                double dis_newLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintY(newBottomc);
                ...
                else
                {
                    ...
                    for (int ci = cellInBinHead; ci <= cellInBinTail; ++ci)
                    {
                        ...
                        {
                            double dis_orgLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintY(cellLoc[boxBCellIds[ci]].Y);
                            double dis_newLoc = cellcol[boxBCellIds[ci]]->calDisToRegionConstraintY(newBottomY + (cellLoc[boxBCellIds[ci]].Y - oriBottomY) * scale);
