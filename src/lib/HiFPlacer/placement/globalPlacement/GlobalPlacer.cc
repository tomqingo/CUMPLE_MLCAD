                                               ...
                                               PlacementTimingOptimizer *timingOptimizer)
{
    ...
    for (int i = 0; i < iterNum || shouldLegalize; i++)
    {
        ...
        placementInfo->UpdateminWLLocation();
        ...
        // converge criteria
        bool criteria0 = upperBoundHPWL / lowerBoundHPWL < 1.3 &&
                         ((averageMacroLegalDisplacement < 4.5 && macroCloseToSite) || macroLegalizationFixed) &&
                         pseudoNetWeight > 0.02;
        bool criteria1 = (progressRatio > 0.98 || (progressRatio > 0.95 && HPWLChangeLittle)) &&
                         ((macroCloseToSite && averageMacroLegalDisplacement < 4.5) || macroLegalizationFixed);
        bool criteria2 = upperBoundHPWL < minHPWL * 2 && macroLegalizationFixed && i > 30;
        bool criteria3 = progressRatio > 0.925 && upperBoundHPWL / minHPWL < 1.3 && macroLegalizationFixed;
        bool criteria4 = iterCntAfterMacrosFixed >= 1 && macroLegalizationFixed;

        if (macroLegalizationFixed)
            iterCntAfterMacrosFixed++;
        if (criteria0 || criteria1 || criteria2 || criteria3 || criteria4 || macroLegalizationFixed)
        {
            std::cout << criteria0 << " " << criteria1 << " | " << criteria2 << " " << criteria3 << " | " << criteria4 << std::endl;
            std::cout << (upperBoundHPWL / lowerBoundHPWL) << " "<<averageMacroLegalDisplacement<<std::endl;
...
void GlobalPlacer::macroLegalize(int curIteration)
{
        ...
        !macroLocked && !macrosBindedToSites && !directMacroLegalize)
    {
        ...
        //Cascade->Simple Macro with Constraints->Simple Macro without Constraints
        BRAMDSPLegalizer->legalize(true, false, false, false);
        BRAMDSPLegalizer->legalize(false, true, false, false);
        BRAMDSPLegalizer->legalize(false, false, false, false);
    ...
    else
    {
            ...
            !macroCloseToSite)
        {
            ...
            else
            {
                ...
                if (averageMCLBLegalDisplacement > 3)
                {
                    print_status("BRAMDSPLegalizer: Launch.");
                    //BRAMDSPLegalizer->resetSitesMapped();
                    placementInfo->getDeviceInfo()->resetBRAMDSPSiteMapping();
                    // Cascade Macro -> Simple Macro With Constraint -> Simple Macro Without Constraint
                    BRAMDSPLegalizer->legalize(true, false, true, directMacroLegalize);
                    BRAMDSPLegalizer->legalize(false, true, false, directMacroLegalize);
                    BRAMDSPLegalizer->legalize(false, false, false, directMacroLegalize);
                    macroCloseToSite = false;
                }
                if (averageCarryLegalDisplacement > 2)
                {
                    if (curIteration % 2 == 0 || averageCarryLegalDisplacement > 5000)
                    {
                        ...
                        CARRYMacroLegalizer->resetSitesMapped();
                        CARRYMacroLegalizer->legalize(false, false, true, directMacroLegalize);
                    }
                    macroCloseToSite = false;
                }
                averageMacroLegalDisplacement = BRAMDSPLegalizer->getAverageDisplacementOfExactLegalization();
...
{
    placementInfo->updateElementBinGrid();
    float supplyRatio = (placementInfo->getBinGridW() < 2.5) ? 0.90 : (0.80 + 0.1 * progressRatio);
    if (!macroLegalizationFixed)
    {
    ...
    //Add the BRAM and DSP in the cell spreading
    // double BRAMDSPspreadingForgetRatio = 0.5;
    // //double BRAMDSPspreadingForgetRatio = spreadingForgetRatio;
    // std::string sharedCellType_BRAM = "RAMB18E2_L";
    // generalSpreader =
    //     new GeneralSpreader(placementInfo, JSONCfg, sharedCellType_BRAM, currentIteration, supplyRatio, verbose);
    // generalSpreader->spreadPlacementUnits(BRAMDSPspreadingForgetRatio);
    // delete generalSpreader;

    // std::string sharedCellType_DSP = "DSP_ALU";
    // generalSpreader =
    //     new GeneralSpreader(placementInfo, JSONCfg, sharedCellType_DSP, currentIteration, supplyRatio, verbose);
    // generalSpreader->spreadPlacementUnits(BRAMDSPspreadingForgetRatio);
    // delete generalSpreader;
