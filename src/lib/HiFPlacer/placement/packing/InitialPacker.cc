...
void InitialPacker::pack()
{
	...
	readMacros(); // read macros from defined cascade shape
    findCARRYMacros();
    findMuxMacros();
    // findBRAMMacros();
    findDSPMacros();
	...
	if (JSONCfg.find("designPath") != JSONCfg.end()) {
    	loadFixedPlacementUnits2(JSONCfg["designPath"] + "/design.pl");
	} else {
    ...
    print_info("#Fixed Placement Units:"+ std::to_string(fixedPlacementUnits.size()));
...
void InitialPacker::readMacros()
{
	// read macros from defined cascade shape
    print_info("Read cascade macro");
	assert(placementMacros.size() == 0);
    float BRAMHeight = 5.0;
    float DSPHeight = 2.5;
    int placementunit_id = 0;
    int simple_macro_cell = 0;
    int cascade_macro_cell = 0;

    std::vector<DesignInfo::DesignCell *> &curCellsInDesign = designInfo->getCells();
    std::map<std::string, PlacementInfo::PlacementMacro *> macroName2macro;

    int curCellsNum = designInfo->getNumCells();

	// check whether cell is in cascade shape
	auto isInCascadeMacro = [](DesignInfo::DesignCell *curCell)
    {
		std::string cellName = curCell->getName();
        if(cellName.find("CASCADE") != cellName.npos && curCell->isMacroType())
            return true;
        else
            return false;
    };
	// split the cascadeCell name by '/'
	auto splitCascadeCellName = [](std::string strorg)
	{
	    std::istringstream iss(strorg);
	    std::vector<std::string> strcol;
	    std::string line;
	    while(getline(iss, line, '/'))
	        strcol.push_back(line);
	    return strcol;
	};
	// get the ID of cell in a cascade shape
	auto getInnerId = [](std::string instName)
	{
		std::string idStr = instName.substr(instName.rfind("_") + 1);
		int i = 0;
		for (; i < idStr.size(); i ++)
			if (idStr[i] >= '0' && idStr[i] <= '9')
				break;
		idStr = idStr.substr(i);
		assert(idStr.size() >= 1);
		// std::cout << instName << " " << idStr << std::endl;
	    return std::stoi(idStr) - 1;
	};

	// traverse all cells
    for(int curCellId = 0; curCellId < curCellsNum; curCellId++)
    {
        auto curCell = curCellsInDesign[curCellId];
        if(!curCell->isMacroType() || cellInMacros.find(curCell) != cellInMacros.end())
            continue; // ff, lut or exist already

        PlacementInfo::PlacementMacro *curMacro;

        if(isInCascadeMacro(curCell))
        {
            std::vector<std::string> namecol;
            namecol = splitCascadeCellName(curCell->getName());
            if(macroName2macro.find(namecol[0]) == macroName2macro.end()) // new shape
            {
				auto macroType = PlacementInfo::PlacementMacro::PlacementMacroType_BRAM;
                if(curCell->isDSP())
					macroType = PlacementInfo::PlacementMacro::PlacementMacroType_DSP;

                curMacro = new PlacementInfo::PlacementMacro(namecol[0], placementunit_id, macroType);

                // curMacro->addOccupiedSite(0, 1);
                curMacro->addCell(curCell, curCell->getCellType(), 0, 0);
                curMacro->SetreferenceCell(curCell->getName());
                cellInMacros.insert(curCell);
                macroName2macro[namecol[0]] = curMacro;
                placementunit_id++;
            }
            else
            {
                curMacro = macroName2macro[namecol[0]];
				int height = curCell->isBRAM() ? BRAMHeight : DSPHeight;
                // curMacro->addOccupiedSite(topoffset, 1);
                curMacro->addCell(curCell, curCell->getCellType(), 0, height * getInnerId(namecol[1]));
                cellInMacros.insert(curCell);
            }
            cascade_macro_cell++;
        }
	...
	// finalize
    for(auto &iter : macroName2macro)
    {
        PlacementInfo::PlacementMacro* curMacro = iter.second;
        int totalWeight = 0;
        for (auto tmpCell : curMacro->getCells())
        {
            ...
            //std::cout<<tmpCell->getName()<<std::endl;
        }
    ...
    print_info("The number of cascaded macro cells: " + std::to_string(cascade_macro_cell));
    print_info("The number of cascaded macro: " + std::to_string(placementMacros.size()));
}
                            ...
                            if (!slotMapping.FFs[i][jj][kk])
                            {
                                if (jj == 0)
                                    slotMapping.FFs[i][jj][kk] = CARRYChain->addVirtualCell(
                                        ...
                                        DesignInfo::CellType_FDRE, 0, CARRYChainSiteOffset);
                                else
                                    slotMapping.FFs[i][jj][kk] = CARRYChain->addVirtualCell(
                    ...
                    if (!slotMapping.FFs[i][1][k])
                    {
                        slotMapping.FFs[i][1][k] =
                            CARRYChain->addVirtualCell(coreCell->getName() + "__FF2" + std::to_string(i * 4 + k),
                                                       designInfo, DesignInfo::CellType_FDRE, 0, CARRYChainSiteOffset);
                    }
                }
            }
                    ...
                    else
                    {
                        ...
                        if (driverPin->getRefPinName().find("CO[") != std::string::npos)
                        {
                            curMacro->addVirtualCell(coreCell->getName() + "__FF2" + std::to_string(FFPinCellId),
                                                     designInfo, DesignInfo::CellType_FDRE, 0, coreOffset);
                            // std::string FFSiteName = std::string(1, FFCode) + "FF2";
                            // outfile0 << "  " << theFF->getName() << " " << CLBSite->getName() << "/" + FFSiteName
                            //          << "\n";
                        }
...
void InitialPacker::findMuxMacros()
{
    ...
    for (int curCellId = 0; curCellId < curNumCells; curCellId++)
    {
        ...
        if (!muxF8HasDirectFF)
            curMacro->addVirtualCell(curCell->getName() + "__FFF8", designInfo, DesignInfo::CellType_FDRE, 0, 0);
        ...
        for (DesignInfo::DesignPin *pinBeDriven : curCell->getInputPins())
        {
            if (pinBeDriven->getRefPinName() == "I0" || pinBeDriven->getRefPinName() == "I1")
            {
                if (!pinBeDriven->getDriverPin())
                {
                    ...
                    curMacro->addVirtualCell(curCell->getName() + "__FFF7", designInfo, DesignInfo::CellType_FDRE, 0,
                                             0);
    ...
    for (int curCellId = 0; curCellId < curNumCells; curCellId++)
    {
        ...
        curMacro->addVirtualCell(curCell->getName() + "__FFF7", designInfo, DesignInfo::CellType_FDRE, 0, 0);
...
void InitialPacker::loadFixedPlacementUnits2(std::string fixedPlacementUnitsFromVivadoFileName)
{
    fixedPlacementUnits.clear();
    std::ifstream infile(fixedPlacementUnitsFromVivadoFileName.c_str());

    std::string line;
    std::string cellName, siteName, BELName;
	int oriSiteX, oriSiteY, oriBelIndex; 
	std::map<int, int> x_; x_[68] = 0; x_[138] = 1;
	auto IOMap = deviceInfo->getIOMap();
    while (std::getline(infile, line))
    {
		if (line == "" || line[0] == '#') continue;
        std::istringstream iss(line);
        iss >> cellName >> oriSiteX >> oriSiteY >> oriBelIndex; 
    
        // int x = x_[oriSiteX];
        // int y = oriSiteY / 30 * 26 + oriBelIndex;
        // assert(y < 260);

		std::tuple<int, int, int> key = std::make_tuple(oriSiteX, oriSiteY, oriBelIndex);


		// siteName = "IOB_X" + std::to_string(x) + "Y" + std::to_string(y);
		siteName = IOMap[key];
        BELName = "HPIOB.INBUF";
        if (cellName.find("OBUF") != cellName.npos)
            BELName = "HPIOB.OUTBUF";
	...
	// std::cout << fixedPlacementUnitsFromVivadoFileName << " size: " << fixedPlacementUnits.size() << std::endl;
	placementInfo->setFixedPUNum(fixedPlacementUnits.size());
}
