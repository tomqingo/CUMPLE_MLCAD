...
void ParallelCLBPacker::dumpDSPBRAMPlacementTcl(std::ofstream &outfileTcl)
{
    for (auto &DSPBRAM_LegalSitePair : placementInfo->getPULegalSite())
    {
        ...
        if (auto tmpMacro = dynamic_cast<PlacementInfo::PlacementMacro *>(DSPBRAM_LegalSitePair.first))
        {
            ...
            {
                std::cout << "Warning: Unknown Cell Type for cell: " << tmpMacro->getName() << "\n";
                // assert(false && "undefined situtation");
            }
        }
                     ...
                     dynamic_cast<PlacementInfo::PlacementUnpackedCell *>(DSPBRAM_LegalSitePair.first))
        {
            ...
            else if (curCell->getOriCellType() == DesignInfo::CellType_RAMB36E2)
            {
                assert(targetSite->getSiteY() % 2 == 0);
                placementStr += "  " + curCell->getName() + " RAMB36_X" +
                            std::to_string(targetSite->getSiteX()) + "Y" +
...
{
	// dump macro only
	std::string macroFile = dumpTclFile.substr(0, dumpTclFile.find(".tcl")) + "-macro.tcl";
    std::ofstream outfileTclMacro(macroFile);
    outfileTclMacro << "set fo [open \"./placementError\" \"w\"]\nplace_design -unplace\nset errorNum 0\n";
    dumpDSPBRAMPlacementTcl(outfileTclMacro);
    outfileTclMacro << "$errorNum\n";
    outfileTclMacro << "# place_design\n# route_design\n";
    outfileTclMacro.close();

	// dump all cells' placement
