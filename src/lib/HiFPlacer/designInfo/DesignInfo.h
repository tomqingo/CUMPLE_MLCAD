...
class DesignInfo
{
  public:
	...
	class MasterCell {
	public:
		MasterCell (std::string name_) : name(name_) {}
		std::string name;
		std::vector<std::string> pinNames;
		std::vector<std::string> pinDirs;
		std::unordered_map<std::string, int> pinName2Id;
	};
    ...
    class DesignCell : public DesignElement
    {
      public:
         ...
         * @param cellType the design cell type (NOT resourse BEL type!)
         * @param id the id of cell in the cell list
		 * @param cellTypeStr_ the original string of cell type
         */
        DesignCell(const std::string &name, DesignCellType cellType, int id, std::string cellTypeStr_)
            : DesignElement(name, ElementType_cell, id), cellType(cellType), cellTypeStr(cellTypeStr_)
        {
            oriCellType = cellType;
            pinPtrs.clear();
            inputPinPtrs.clear();
        ...
        inline bool isBUFG()
        {
            return cellType == CellType_BUFGCE;
        }

        inline bool isInMacro()
        {
            return isDSP() || isBRAM();
        }
        inline bool isInCascadeMacro()
        {
            std::string cellname = getName();
            if((cellname.find("CASCADE") != cellname.npos) && isInMacro())
                return true;
            else
                return false;
        }
        ...
        inline bool hasRegionConstraint()
        {
            return flag_isinRegionConstraint;
        }

        inline double calDisToRegionConstraintX(float locX)
        {
            double min_dis = 1000000;
            double region_slack = 0.5;
            for(int i=0; i<regionconstraint.size(); i++)
            {
                double x_L = regionconstraint[i][0];
                double x_H = regionconstraint[i][1];
                double dis = 0;
                if(x_L+region_slack<locX && locX<x_H-region_slack)
                    dis = 0;
                else
                {
                    if(locX>=x_H-region_slack)
                        dis = locX-(x_H-region_slack);
                    else
                        dis = (x_L+region_slack)-locX;
                }
                if(dis < min_dis)
                    min_dis = dis;
            }
        ...
        inline double calDisToRegionConstraintY(float locY)
        {
            double min_dis = 1000000;
            double region_slack = 1.0;
            if(isDSP())
                region_slack = 2.5;
            else
            {
            ...
            for(int i=0; i<regionconstraint.size(); i++)
            {
                double y_L = regionconstraint[i][2];
                double y_H = regionconstraint[i][3];
                double dis = 0;
                if(y_L+region_slack<locY && locY<y_H-region_slack)
                    dis = 0;
                else
                {
                    if(locY>=y_H-region_slack)
                        dis = locY-(y_H-region_slack);
                    else
                        dis = (y_L+region_slack)-locY;
                }
                if(dis < min_dis)
                    min_dis = dis;
            }
        ...
        inline bool IsinRegionConstraint(float locX, float locY)
        {
            bool flag_inRegion = false;
            double left_right_region_slack = 0.5;
            double up_down_region_slack = 1.0;
            if(isDSP())
            {
                up_down_region_slack = 2.5;
            }
            else
            {
                if(isBRAM())
            ...
            for(int i=0; i<regionconstraint.size(); i++)
            {
                double xL = regionconstraint[i][0];
                double xH = regionconstraint[i][1];
                double yL = regionconstraint[i][2];
                double yH = regionconstraint[i][3];

                if((xL+left_right_region_slack<=locX && locX <=xH-left_right_region_slack) && (yL+up_down_region_slack<=locY && locY <= yH-up_down_region_slack))
                    flag_inRegion = true;
            }

            return flag_inRegion;
        }
        ...
        inline void setRegionConstrType(int constrtypeid)
        {
            regionconstrtype = constrtypeid;
        }

        inline int getRegionConstrType()
        {
            return regionconstrtype;
        }

        inline std::vector<std::vector<double>> getRegionConstr()
        {
            return regionconstraint;
        }

        inline void AddRegionalConstraint(double xL, double xH, double yL, double yH)
        {
            std::vector<double> regionconstraintsub;
            regionconstraintsub.push_back(xL);
            regionconstraintsub.push_back(xH);
            regionconstraintsub.push_back(yL);
            regionconstraintsub.push_back(yH);
            regionconstraint.push_back(regionconstraintsub);
            flag_isinRegionConstraint = true;
        }
        ...
        inline void SetLoc(float locX_in, float locY_in)
        {
            locX = locX_in;
            locY = locY_in;
        }

        inline float GetCellLocX()
        {
            return locX;
        }

        inline float GetCellLocY()
        {
            return locY;
        }
      ...
      private:
        ...
        //Judge whether the node is region constrained
        bool flag_isinRegionConstraint = false;
        int regionconstrtype = -1;
        std::vector<std::vector<double>> regionconstraint;

        //add the location information of cell
        float locX = 0.0;
        float locY = 0.0;

        //Judge whether the node is fixed or not
        bool fixed = false;
    ...
    /**
     * @brief class for constrained area on FPGA (design-specific)
     *
     * DesignRegionConstr is a concept in the FPGA to constrain a number of simple non-cascade nodes in a specific region
     *
     */
    class DesignRegionConstr
    {
        ...
        DesignRegionConstr(int id, int num_boxes): id(id), num_boxes(num_boxes)
        {

        }

        inline void AddBox(double xL, double xH, double yL, double yH)
        {
            std::vector<double> constrcolsub;
            constrcolsub.push_back(xL);
            constrcolsub.push_back(xH);
            constrcolsub.push_back(yL);
            constrcolsub.push_back(yH);
            constrcol.push_back(constrcolsub);            
        }

        inline int GetRegionConstrId()
        {
            return id;
        }


        inline int GetBoxNum()
        {
            return num_boxes;
        }

        inline std::vector<double> GetBoxInfo(int box_id)
        {
            return constrcol[box_id];
        }

        inline std::vector<int> GetbeIncludedList()
        {
            std::vector<int> beIncludedListId;
            for(int i=0; i < beIncludedList.size(); i++)
                beIncludedListId.push_back(beIncludedList[i]->GetRegionConstrId());
            return beIncludedListId;
        }

        inline bool hasRegionConstrainedCell(DesignCell* designcell)
        {
            return designcellregionconstrained.find(designcell) != designcellregionconstrained.end();
        }

        inline void addSite(std::string sharedbeltype, int belnumpertype)
        {
            if(bel_supply.find(sharedbeltype) == bel_supply.end())
                bel_supply[sharedbeltype] = belnumpertype;
            else
                bel_supply[sharedbeltype] += belnumpertype;
        }

        inline void addNode(DesignCell* designcell, std::string sharedBELType, bool isRegionalConstrainedNode)
        {
            designcellcol.push_back(designcell);
            //celloccupationcol.push_back(celloccupation);
            if(bel_occupy.find(sharedBELType) == bel_occupy.end())
                bel_occupy[sharedBELType] = 1;
            else
                bel_occupy[sharedBELType] += 1;
            if(cellType2belType.find(designcell->getCellType()) == cellType2belType.end())
                cellType2belType[designcell->getCellType()] = sharedBELType;
            
            if(isRegionalConstrainedNode)
            {
                designcellregionconstrained.insert(designcell);
                for(int i=0; i<beIncludedList.size(); i++)
                {
                    if(!beIncludedList[i]->hasRegionConstrainedCell(designcell))
                        beIncludedList[i]->addNode(designcell, sharedBELType, true);
                }
                num_nodes_required += 1;
            }
            else
            {
                num_nodes_not_required += 1;
            }
        }       

        inline bool CheckIsFull(std::string sharedbeltype, double ratio=1.0)
        {
            return bel_occupy[sharedbeltype] >= bel_supply[sharedbeltype]*ratio;
        }

        inline void resetNodeCol()
        {
            while(num_nodes_not_required>0)
            {
                DesignCell* celllast = designcellcol.back();
                designcellcol.pop_back();
                std::string sharedBELType = cellType2belType[celllast->getCellType()];
                //int celloccupation = celloccupationcol.back();
                //celloccupationcol.pop_back();
                bel_occupy[sharedBELType] -= 1;
                num_nodes_not_required--;
            }
        }

        inline bool IsinRegionConstraint(double locX, double locY, double left_right_slack, double up_down_slack)
        {
            
            bool flag_isInRegion = false;
            for(int i=0; i<constrcol.size(); i++)
            {
                double xL = constrcol[i][0];
                double xH = constrcol[i][1];
                double yL = constrcol[i][2];
                double yH = constrcol[i][3];
                //std::cout<<locX<<' '<<locY<<' '<<xL<<' '<<xH<<' '<<yL<<' '<<yH<<std::endl;

                if((xL+left_right_slack<=locX && locX<=xH-left_right_slack) && (yL+up_down_slack<=locY && locY<=yH-up_down_slack))
        ...
        inline bool IsCovereRange(double xLb, double xHb, double yLb, double yHb)
        {
            int boxnum = GetBoxNum();
            for(int i=0; i < boxnum; i++)
            {
                ...
                double yL = constrcol[i][2];
                double yH = constrcol[i][3];
                if((xL <= xLb) && (xH >= xHb) && (yL <= yLb) && (yH >= yHb))
                    return true;
            }
            return false;
        }

        inline bool IsBeIncluded(DesignRegionConstr* regionconstrb)
        {
            int boxnum = GetBoxNum();
            ...
            for(int i=0; i < boxnum; i++)
            {
                ...
                if(!regionconstrb->IsCovereRange(xL, xH, yL, yH))
                    return false;
           }

            beIncludedList.push_back(regionconstrb);
            return true;
        }

        inline std::map<std::string, int> GetBELOccupy()
        {
            return bel_occupy;
        }

        inline std::map<std::string, int> GetBELSupply()
        {
            return bel_supply;
        }        
        ...
        private:
            int id;
            int num_boxes = 0;
            int num_nodes_required = 0;
            int num_nodes_not_required = 0;
            std::vector<std::vector<double>> constrcol;
            std::vector<DesignCell*> designcellcol;
            std::set<DesignCell*> designcellregionconstrained;
            //std::vector<int> celloccupationcol;
            std::map<std::string, int> bel_occupy;
            std::map<std::string, int> bel_supply;
            std::map<DesignCellType, std::string> cellType2belType;
            std::vector<DesignRegionConstr*> beIncludedList;        
    };
    ...
    void DesignInfo_ori(std::map<std::string, std::string> &JSONCfg, DeviceInfo *deviceInfo);
    void DesignInfo_new(std::map<std::string, std::string> &JSONCfg, DeviceInfo *deviceInfo);
    ~DesignInfo()
    {
    ...
    // Add the region constr to the designconstrs
    inline void AddRegionalConstraint(DesignRegionConstr* designconstr_per)
    {
        designconstrs.push_back(designconstr_per);
    }

    // Get the regional constraint from the designconstrs
    inline DesignRegionConstr* GetRegionalConstraint(int regiontype_id)
    {
        return designconstrs[regiontype_id];
    }

    inline std::vector<DesignRegionConstr *> GetRegionConstraintAll()
    {
        return designconstrs;
    }
  ...
  private:
    ...
    std::vector<DesignRegionConstr *> designconstrs;
    std::map<std::string, DesignNet *> name2Net;
    std::map<std::string, DesignCell *> name2Cell;
    std::map<std::string, DesignCell *> name2Celltype;
    std::map<std::string, int> name2pinid;
